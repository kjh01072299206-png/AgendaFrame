...
2026-08-03 현행화 기준: 구현(2.x~5.x)은 원 계획보다 3주 이상 앞당겨 사실상 완료됐다.
수집 방식은 Playwright 상시 크롤링에서 BigKinds 메타데이터 가져오기 + 홈페이지 배치
관측으로 피벗했고, 프레임 코드북은 6종 태그에서 문제정의·원인·책임·평가·해법+취재원
구조로 교체됐다. 남은 임계 경로는 구현이 아니라 **① 실제 사람 라벨 기반 품질 실측과
릴리스 게이트 통과, ② 최신 빌드의 공개 URL 배포**다.
...
| 4.0 | 의제 점수·대시보드 | 점수 산식·가중치 정의 | 최지우 | 산식(다양성 45·기사량 30·배치 15·후속 10) | 2026-07-24 | 2026-07-31 | 완료 — 문서 표기 불일치 해소 필요(5.6) |
...
| 5.3 | 프레임·AI 리포트 | 프레임 수동 검증 실측 | 공동 | 이중 코딩·adjudication 보고서 | 2026-08-04 | 2026-08-17 | **진행 예정 (스프린트 5)** |
| 5.4 | 프레임·AI 리포트 | 리포트 구현 | 공동 | 규칙 기반 리포트·챗봇 | 2026-07-28 | 2026-08-02 | 완료 (LLM 리포트는 오프라인 경로) |
| 5.5 | 배포 정합성 | v6 빌드 스테이징·공개 URL 승격 | 강준혁 | 라이브 v6 + initial-five 공개 | 2026-08-04 | 2026-08-10 | **대기 — 임계 경로** (라이브는 v5, 신규 라우트 404) |
| 5.6 | 배포 정합성 | 가중치 표기·stale 문서 정합화 | 공동 | 단일 산식 문서 | 2026-08-04 | 2026-08-08 | 대기 |
...
```mermaid
gantt
    title AgendaFrame WBS (2026-08-03 현행화)
    ...
    v6 재배포·라우트 공개          :crit, f1, 2026-08-04, 7d
    가중치·stale 문서 정합화       :f2, 2026-08-04, 5d
    holdout 라이선스·이중 코딩     :crit, f3, 2026-08-04, 10d
