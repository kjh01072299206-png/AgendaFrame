...
function publicComparisonFromEngine(rawComparison, profiles, issueArticles, { issueArticleCount = issueArticles.length, approval = null } = {}) {
  ...
  const analysisModules = rawModules ? {
    ...
    genericFrames: {
      status: rawModules.generic_frames?.status ?? "partial",
      methodVersion: rawModules.generic_frames?.method_version ?? FRAMING_ENGINE_VERSION,
      taxonomyVersion: rawModules.generic_frames?.taxonomy_version ?? "generic-frames-v1",
      unit: rawModules.generic_frames?.unit ?? "article_presence",
      byOutlet: (rawModules.generic_frames?.by_outlet ?? []).map((entry) => ({
        source: String(entry.outlet),
        analyzedArticles: Number(entry.analyzed_article_count ?? 0),
        frames: (entry.frames ?? []).map((frame) => ({
          code: String(frame.code),
          label: String(frame.label),
          articleCount: Number(frame.article_count ?? 0),
          present: frame.present === true,
          sentenceCount: Number(frame.sentence_count ?? 0),
          evidenceRefs: publicEvidenceRefs(frame.evidence),
        })),
      })),
      caution: rawModules.generic_frames?.caution ?? null,
    },
    compositionClusters: {
      status: rawModules.composition_clusters?.status ?? "partial",
      methodVersion: rawModules.composition_clusters?.method_version ?? FRAMING_ENGINE_VERSION,
      basis: rawModules.composition_clusters?.basis ?? "article_level_entman_function_presence",
      clusters: (rawModules.composition_clusters?.clusters ?? []).map((cluster) => ({
        ...
        outletCount: Number(cluster.outlet_count ?? 0),
        articleIds: (cluster.article_ids ?? []).map(String),
        outlets: (cluster.outlets ?? []).map(String),
        observedDimensions: (cluster.observed_dimensions ?? []).map(String),
        evidenceRefs: publicEvidenceRefs(cluster.evidence),
      })),
      caution: rawModules.composition_clusters?.caution ?? null,
    },
    semanticNetworks: {
      status: rawModules.semantic_networks?.status ?? "partial",
      methodVersion: rawModules.semantic_networks?.method_version ?? FRAMING_ENGINE_VERSION,
      basis: rawModules.semantic_networks?.basis ?? "same_sentence_controlled_concept_cooccurrence",
      groups: (rawModules.semantic_networks?.groups ?? []).map((group) => ({
        clusterId: String(group.cluster_id),
        title: String(group.title),
        outlets: (group.outlets ?? []).map(String),
        articleCount: Number(group.article_count ?? 0),
        ...
        edges: (group.edges ?? []).map((edge) => ({
          ...
          sentenceCount: Number(edge.sentence_count ?? 0),
        })),
      })),
      limitations: (rawModules.semantic_networks?.limitations ?? []).map(String),
    },
    devices: {
      status: rawModules.devices?.status ?? "partial",
      methodVersion: rawModules.devices?.method_version ?? FRAMING_ENGINE_VERSION,
      byOutlet: (rawModules.devices?.by_outlet ?? []).map((entry) => ({
        source: String(entry.outlet),
        analyzedArticles: Number(entry.analyzed_article_count ?? 0),
        headlineAlignment: entry.headline_alignment ? String(entry.headline_alignment) : null,
        devices: (entry.devices ?? []).map((device) => ({
          code: String(device.code),
          label: String(device.label),
          articleCount: Number(device.article_count ?? 0),
          count: Number(device.count ?? 0),
          leadArticleCount: Number(device.lead_article_count ?? 0),
          evidenceRefs: publicEvidenceRefs(device.evidence),
        })),
      })),
      caution: rawModules.devices?.caution ?? null,
    },
    reportingStyle: {
