...
function groundedAnswer(bundle: IssueAnalysisBundle, question: string) {
  ...
  if (/취재원|화자|인용/.test(question) || (/누가|누구/.test(question) && !namedDimension)) {
    const actors = new Map<
      string,
      { count: number; articleId: string; evidence?: PublicEvidence; outlets: Set<string>; said?: string; subjects: Set<string> }
    >();
    for (const entry of profiles) {
      /* 화자는 역할 코드로만 말할 수 있다. 코딩 지침이 실명 반환을 금지하고, 의역문에서 좁힌 값은
         그 문장이 다루는 대상이라 화자와 다르다 — "법치를 흔드는 사안 앞에서 책임을 피한다" 는
         대통령을 평가한 문장이지 대통령이 한 말이 아니다. 그래서 대상은 따로 적는다. */
      const paraAt = paraphrasesByLocator(entry.profile.dimensions);
      for (const actor of entry.profile.actors_and_sources ?? []) {
        const label = actor.role_label ?? actor.role ?? "기타 취재원";
        const texts = actorParaphrases(actor, paraAt);
        const subject = narrowSubject(texts);
        const current = actors.get(label) ?? {
          count: 0,
          articleId: entry.articleId,
        ...
        // 무엇을 실었는지까지 붙인다 — 역할 이름만으로는 입장을 알 수 없다
