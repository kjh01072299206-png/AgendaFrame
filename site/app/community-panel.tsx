...
import { COMMUNITY_API_ENABLED, communityFetch } from "./(shell)/community-session";
...
export function CommunityPanel({ issueId }: { issueId: string }) {
  ...
  const load = useCallback(async () => {
    ...
    try { const response = await communityFetch(`/api/issues/${encodeURIComponent(issueId)}/community`, { cache: "no-store" }); const payload = await response.json(); if (response.ok) setComments(payload.comments ?? []); else setNotice(payload?.error?.message ?? "댓글을 불러오지 못했습니다."); } catch { setNotice("댓글을 불러오지 못했습니다."); }
  }, [issueId]);
  // The fetch is an external synchronization for the selected issue.
  // eslint-disable-next-line react-hooks/set-state-in-effect
  useEffect(() => { void load(); }, [load]);
  const submit = async (event: FormEvent) => {
    event.preventDefault(); if (!body.trim() || loading) return; if (!COMMUNITY_API_ENABLED) { setNotice("커뮤니티 기능이 아직 연결되지 않았습니다."); return; } setLoading(true); setNotice("");
    try { const response = await communityFetch(`/api/issues/${encodeURIComponent(issueId)}/community`, { method: "POST", body: JSON.stringify({ body, displayName }) }); const payload = await response.json(); if (!response.ok) throw new Error(payload?.error?.message ?? "댓글을 등록하지 못했습니다."); setComments((current) => [...current, payload.comment]); setBody(""); } catch (error) { setNotice(error instanceof Error ? error.message : "댓글을 등록하지 못했습니다."); } finally { setLoading(false); }
  };
  const report = async (id: string) => { if (!COMMUNITY_API_ENABLED) { setNotice("커뮤니티 기능이 아직 연결되지 않았습니다."); return; } try { const response = await communityFetch(`/api/comments/${encodeURIComponent(id)}/report`, { method: "POST", body: JSON.stringify({}) }); const payload = await response.json(); setNotice(response.ok ? "신고가 접수되었습니다. 검토 후 조치합니다." : payload?.error?.message ?? "신고를 접수하지 못했습니다."); } catch { setNotice("신고를 접수하지 못했습니다."); } };
  return (
