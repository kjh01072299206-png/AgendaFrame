...
type Issue = {
  ...
  evidenceBasis: "headline_metadata_only" | "body_signals_and_metadata";
};
...
type Comparison = {
  ...
  evidenceBasis: "headline_metadata_only" | "body_signals_not_structured_comparison" | "evidence_spans";
  reason: string;
      ...
      <main id="main-content" tabIndex={-1}>
        <section className="overview" aria-labelledby="hero-title">
          ...
          <dl className="overview-status" aria-label="현재 분석 범위">
            ...
            <div><dt>현재 근거</dt><dd>{bodyEvidenceCount ? `본문 기반 단서 ${bodyEvidenceCount.toLocaleString("ko-KR")}건` : "제목·배치 메타데이터"}</dd></div>
                  ...
                  <div id="analysis-panel-compare" role="tabpanel" aria-labelledby="analysis-tab-compare" className="evidence-first">
                    ...
                    <div className="evidence-grid">
                      ...
                      <section>
                        ...
                        <h4>누구의 목소리가 실렸나</h4>
                        {detail.comparison.sourceVoices.length ? detail.comparison.sourceVoices.map((voice) => <article key={`${voice.sourceType}-${voice.supports}`}><strong>{voice.sourceType}</strong><p>{voice.supports}</p></article>) : <p className="withheld">본문 근거가 없거나 취재원 추출이 검토되지 않아 발언 주체와 맥락을 판정하지 않습니다.</p>}
                      </section>
                      <section>
