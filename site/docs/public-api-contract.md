...
기본 수집 범위는 제목·원문 URL·매체·섹션·게시/수집 시각·반복 관측된 홈페이지 배치다. 자동 원문 분석은 공개 기사 HTML을 Worker 메모리에서 한 번만 처리하고, 기사 전문·HTML·원문 문장을 D1·R2·로그에 쓰지 않는다. 공개 API는 구조화된 프레임 단서와 원문 링크만 반환한다. 정식 라이선스 자료의 별도 보관은 호환용 `POST /api/content` 경로로 분리한다.
...
제목만으로 위 내용을 추정하지 않는다. 임시 본문이 있어도 현재 규칙 공급자는 표현 단서만 검출하며, 원문 문장을 보존하지 않으므로 구조화 비교와 사람 검토가 끝나기 전에는 원인·책임·해법·취재원 비교를 보류한다. 제목 또는 임시 본문에서 검출되지 않은 프레임 요소는 0점과 `not_detected`로 기록하며, 확률이나 신뢰도처럼 표시하지 않는다.
...
- `POST /api/analyze/transient`: 등록된 공식 기사 URL을 제한 요청하고 메모리에서 분석한 뒤 전문 없이 구조화 결과만 저장
