...
현재 수집 범위는 제목·원문 URL·매체·섹션·게시/수집 시각·관측된 홈페이지 배치뿐이다. 기사 본문은 저장하지 않는다.
...
제목만으로 위 내용을 추정하지 않는다. 제목에서 검출되지 않은 프레임 요소는 0점과 `not_detected`로 기록하며, 확률이나 신뢰도처럼 표시하지 않는다.
