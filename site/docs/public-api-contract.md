...
기본 수집 범위는 제목·원문 URL·매체·섹션·게시/수집 시각·반복 관측된 홈페이지 배치다. 자동 원문 분석은 공개 기사 HTML을 Worker 메모리에서 처리한 뒤 기사 전문·HTML·원문 문장을 D1·R2·로그에 쓰지 않는다. 기사별로 분석 상태, 비복원 해시, 글자 수, 감지된 프레임 코드와 분석 버전만 저장해 중단된 다음 배치가 이어서 처리할 수 있게 한다. 공개 API는 구조화된 프레임 단서와 원문 링크만 반환한다. 정식 라이선스 자료의 별도 보관은 호환용 `POST /api/content` 경로로 분리한다.
